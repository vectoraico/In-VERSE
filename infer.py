...
parser.add_argument('-i', '--input', type=str, help='input panorama image')
parser.add_argument('--input_dir', type=str, help='input panorama image dir')
parser.add_argument('-o', '--output',  default='./output', type=str, help='output path')
...
def predict(model, input_path, output_dir):
    ...
    vis.save(os.path.join(output_dir, os.path.splitext(os.path.basename(input_path))[0] + "_vis.jpg"))

    # Save output 3d layout as json
    Layout.saveSceneAsJson(os.path.join(output_dir, os.path.splitext(os.path.basename(input_path))[0] + "_res.json"), scene_pred)
...
if __name__ == '__main__':
    ...
    if args.input_dir and len(args.input_dir) != 0:
        for image_name in os.listdir(args.input_dir):
            ...
            predict(model, input_path, args.output)
    else:

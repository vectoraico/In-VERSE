...
def inference():
    ...
    for img_path in bar:
        ...
        if args.post_processing is not None and 'manhattan' in args.post_processing:
            ...
            img, vp = preprocess(img, vp_cache_path=os.path.join(args.output_dir, f"raw/{name}_vp.txt"))
...
def run_one_inference(img, model, args, name, logger, show=False, show_depth=True,
    ...
    visualize_2d(img, dt,
                 ...
                 show=show,
                 save_path=os.path.join(args.output_dir, f"raw/{name}_pred.png"))
    ...
    json_data = save_pred_json(output_xyz, tensor2np(dt['ratio'][0])[0],
                               save_path=os.path.join(args.output_dir, f"raw/{name}_pred.json"))
    ...
    output_data = {
        ...
        'dt_layout_depth': dt_layout_depth,
        'save_path': os.path.join(args.output_dir, f"3D/{name}_3d{mesh_format}"),
    }

    output_file_path = os.path.join(args.output_dir, f'pkl/{name}.pkl')
